...
def readData():
    ...
    df_image = pd.read_csv("./data/database_image.csv")
...
def query(text = None, image = None):
    ...
    pareser.add_argument('--weight', type=str, help = "Path to the model weight file")
    args = pareser.parse_args()
    lat, longt, location = getLocation()
    db_img, db_text, df_image, df_text = readData()
    model, processor, tokenizer = load_model(weight = args.weight)
    model.eval()
    process = None
    input_features = None
    D, I = None, None
    ...
    if image != None:
        ...
        save_dir_ = './segment_text' # Load the word and sentence segmentation component
        if rdrsegmenter is None:
            rdrsegmenter = py_vncorenlp.VnCoreNLP(annotators=["wseg"], save_dir = save_dir_)
    ...
    print(results)
    df_infor = pd.read_csv("./data/infor.csv")
    informations = df_infor["information"].tolist()
    ...
    return data, location, informations[results[0]]
...
def index():
    if request.method == 'POST':
        ...
        if not text and not image_url:
            ...
            df = pd.read_csv("./data/data3.csv")
            distance = calculateDistance(df = df, lat = lat, lng = lng)
            df['Distance'] = distance
            data = df[['Image', 'Information', 'Rating', 'Address', 'Distance']]
            data['Rating'] = data['Rating'].astype(float)
            data = data.sort_values("Distance", axis = 0, ascending = True)
            data = data.to_dict(orient = "records")
            return render_template('index.html', data = data, location = location)
        data, location, information = query(text = text, image = image)
        
        return render_template('index.html', text = text, image_url = image_url, data = data, location = location, information = information)
